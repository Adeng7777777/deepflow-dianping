...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const result = await new Promise((resolve, reject) => {
        ...
        (apiRes) => {
          ...
          apiRes.on("end", () => {
            try {
              ...
              resolve({ status: apiRes.statusCode, payload });
            } catch (e) {
              reject(new Error("DeepSeek 返回数据解析失败"));
            }
          });
        }
      );
      ...
      apiReq.on("error", (e) => {
        reject(new Error("DeepSeek 请求失败: " + e.message));
      });
    ...
    if (result.status !== 200) {
      return res.status(result.status).json({
        error: result.payload?.error?.message || "DeepSeek 请求失败",
      });
    }
